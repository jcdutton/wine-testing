...
static void attach_cbpf(int fd)
{
	struct sock_filter code[] = {
		/* A = 0 */ // Like Windows sockets
		{ BPF_LD  | BPF_W | BPF_IMM, 0, 0, 0 },
		/* A = 1 */ // Like Linux sockets
		//{ BPF_LD  | BPF_W | BPF_IMM, 0, 0, 1 },
		/* return A */
		{ BPF_RET | BPF_A, 0, 0, 0 },
		};
	struct sock_fprog p = {
...
int main() {
	...
	int enable = 1;
	if (setsockopt(sockfd1, SOL_SOCKET, SO_REUSEPORT, &enable, sizeof(int)) < 0) {
		perror("setsockopt(SO_REUSEADDR) failed on sockfd1");
	}
	enable = 1;
	if (setsockopt(sockfd2, SOL_SOCKET, SO_REUSEPORT, &enable, sizeof(int)) < 0) {
